...
const defaultColors = ['#001a1a', '#00E5FF', '#003344'];
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      private updateHoverState(cx: number, cy: number) {
        this.isHoverInside = this.isPointInside(cx, cy); return this.isHoverInside;
      }
      ...
      onDocumentMouseMove(event: MouseEvent) {
        ...
        if (this.isAutoActive && !this.hasUserControl && !this.takeoverActive) {
          if (!this.container) return;
          const r = this.container.getBoundingClientRect();
          const nx = (event.clientX - r.left) / r.width; const ny = (event.clientY - r.top) / r.height;
    ...
    class ShaderPass {
      ...
      init() {
        ...
        if (this.uniforms) {
          this.material = new THREE.RawShaderMaterial(this.props.material as object);
    ...
    class Advection extends ShaderPass {
      ...
      update(args: { dt?: number; isBounce?: boolean; BFECC?: boolean } = {}) { if (!this.uniforms) return; if (typeof args.dt === 'number') this.uniforms.dt.value = args.dt; if (typeof args.isBounce === 'boolean') this.line.visible = args.isBounce; if (typeof args.BFECC === 'boolean') this.uniforms.isBFECC.value = args.BFECC; super.update(); }
    }

    class ExternalForce extends ShaderPass {
      mouse!: THREE.Mesh;
      constructor(simProps: Record<string, unknown>) { super({ output: simProps.dst }); this.init(simProps); }
      init(simProps?: Record<string, unknown>) { super.init(); const mG = new THREE.PlaneGeometry(1, 1); const mM = new THREE.RawShaderMaterial({ vertexShader: mouse_vert, fragmentShader: externalForce_frag, blending: THREE.AdditiveBlending, depthWrite: false, uniforms: { px: { value: simProps?.cellScale }, force: { value: new THREE.Vector2(0, 0) }, center: { value: new THREE.Vector2(0, 0) }, scale: { value: new THREE.Vector2(simProps?.cursor_size as number, simProps?.cursor_size as number) } } }); this.mouse = new THREE.Mesh(mG, mM); this.scene!.add(this.mouse); }
      update(props: Record<string, unknown> = {}) { const fx = (Mouse.diff.x / 2) * (props.mouse_force as number || 0); const fy = (Mouse.diff.y / 2) * (props.mouse_force as number || 0); const cs = props.cellScale as THREE.Vector2 || new THREE.Vector2(1,1); const cur = props.cursor_size as number || 0; const cx = Math.min(Math.max(Mouse.coords.x, -1 + cur * cs.x + cs.x * 2), 1 - cur * cs.x - cs.x * 2); const cy = Math.min(Math.max(Mouse.coords.y, -1 + cur * cs.y + cs.y * 2), 1 - cur * cs.y - cs.y * 2); const u = (this.mouse.material as THREE.RawShaderMaterial).uniforms; u.force.value.set(fx, fy); u.center.value.set(cx, cy); u.scale.value.set(cur, cur); super.update(); }
    }

    class Viscous extends ShaderPass {
      constructor(sp: Record<string, unknown>) { super({ material: { vertexShader: face_vert, fragmentShader: viscous_frag, uniforms: { boundarySpace: { value: sp.boundarySpace }, velocity: { value: (sp.src as THREE.WebGLRenderTarget).texture }, velocity_new: { value: (sp.dst_ as THREE.WebGLRenderTarget).texture }, v: { value: sp.viscous }, px: { value: sp.cellScale }, dt: { value: sp.dt } } }, output: sp.dst, output0: sp.dst_, output1: sp.dst }); this.init(); }
      update(args: { viscous?: number; iterations?: number; dt?: number } = {}) { if (!this.uniforms) return; let fo: THREE.WebGLRenderTarget | undefined, fi: THREE.WebGLRenderTarget | undefined; if (typeof args.viscous === 'number') this.uniforms.v.value = args.viscous; const it = args.iterations ?? 0; for (let i = 0; i < it; i++) { fi = i % 2 === 0 ? this.props.output0 as THREE.WebGLRenderTarget : this.props.output1 as THREE.WebGLRenderTarget; fo = i % 2 === 0 ? this.props.output1 as THREE.WebGLRenderTarget : this.props.output0 as THREE.WebGLRenderTarget; this.uniforms.velocity_new.value = fi.texture; this.props.output = fo; if (typeof args.dt === 'number') this.uniforms.dt.value = args.dt; super.update(); } return fo; }
    }

    class Divergence extends ShaderPass {
      constructor(sp: Record<string, unknown>) { super({ material: { vertexShader: face_vert, fragmentShader: divergence_frag, uniforms: { boundarySpace: { value: sp.boundarySpace }, velocity: { value: (sp.src as THREE.WebGLRenderTarget).texture }, px: { value: sp.cellScale }, dt: { value: sp.dt } } }, output: sp.dst }); this.init(); }
    ...
    class Poisson extends ShaderPass {
      constructor(sp: Record<string, unknown>) { super({ material: { vertexShader: face_vert, fragmentShader: poisson_frag, uniforms: { boundarySpace: { value: sp.boundarySpace }, pressure: { value: (sp.dst_ as THREE.WebGLRenderTarget).texture }, divergence: { value: (sp.src as THREE.WebGLRenderTarget).texture }, px: { value: sp.cellScale } } }, output: sp.dst, output0: sp.dst_, output1: sp.dst }); this.init(); }
      update(args: { iterations?: number } = {}) { let pi: THREE.WebGLRenderTarget | undefined, po: THREE.WebGLRenderTarget | undefined; const it = args.iterations ?? 0; for (let i = 0; i < it; i++) { pi = i % 2 === 0 ? this.props.output0 as THREE.WebGLRenderTarget : this.props.output1 as THREE.WebGLRenderTarget; po = i % 2 === 0 ? this.props.output1 as THREE.WebGLRenderTarget : this.props.output0 as THREE.WebGLRenderTarget; if (this.uniforms) this.uniforms.pressure.value = pi.texture; this.props.output = po; super.update(); } return po; }
    }

    class Pressure extends ShaderPass {
      constructor(sp: Record<string, unknown>) { super({ material: { vertexShader: face_vert, fragmentShader: pressure_frag, uniforms: { boundarySpace: { value: sp.boundarySpace }, pressure: { value: (sp.src_p as THREE.WebGLRenderTarget).texture }, velocity: { value: (sp.src_v as THREE.WebGLRenderTarget).texture }, px: { value: sp.cellScale }, dt: { value: sp.dt } } }, output: sp.dst }); this.init(); }
      update(args: { vel?: THREE.WebGLRenderTarget; pressure?: THREE.WebGLRenderTarget } = {}) { if (this.uniforms && args.vel && args.pressure) { this.uniforms.velocity.value = args.vel.texture; this.uniforms.pressure.value = args.pressure.texture; } super.update(); }
    }
    ...
    class Simulation {
      ...
      constructor(options?: Partial<SimOptions>) { this.options = { iterations_poisson: 32, iterations_viscous: 32, mouse_force: 20, resolution: 0.5, cursor_size: 100, viscous: 30, isBounce: false, dt: 0.014, isViscous: false, BFECC: true, ...options }; this.init(); }
      ...
      createAllFBO() { const type = this.getFloatType(); const opts = { type, depthBuffer: false, stencilBuffer: false, minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, wrapS: THREE.ClampToEdgeWrapping, wrapT: THREE.ClampToEdgeWrapping } as const; for (const key in this.fbos) this.fbos[key] = new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y, opts); }
      createShaderPass() {
      ...
      update() {
        ...
        if (this.options.isViscous) { const v = this.viscous.update({ viscous: this.options.viscous, iterations: this.options.iterations_viscous, dt: this.options.dt }); if (v) vel = v; }
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: Record<string, unknown>) {
        this.props = props; Common.init(props.$wrapper as HTMLElement); Mouse.init(props.$wrapper as HTMLElement);
        Mouse.autoIntensity = props.autoIntensity as number; Mouse.takeoverDuration = props.takeoverDuration as number;
      ...
      dispose() { try { window.removeEventListener('resize', this._resize); if (this._onVisibility) document.removeEventListener('visibilitychange', this._onVisibility); Mouse.dispose(); if (Common.renderer) { const c = Common.renderer.domElement; if (c?.parentNode) c.parentNode.removeChild(c); Common.renderer.dispose(); Common.renderer.forceContextLoss(); } } catch { /* noop */ } }
    }
