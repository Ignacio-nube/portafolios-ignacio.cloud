...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  return (
    <>
      <style>{`
        :root { --linear-ease: linear(0,0.068,0.19 2.7%,0.804 8.1%,1.037,1.199 13.2%,1.245,1.27 15.8%,1.274,1.272 17.4%,1.249 19.1%,0.996 28%,0.949,0.928 33.3%,0.926,0.933 36.8%,1.001 45.6%,1.013,1.019 50.8%,1.018 54.4%,1 63.1%,0.995 68%,1.001 85%,1); --color-1: #00E5FF; --color-2: #0099bb; --color-3: #003344; --color-4: #00ccdd; }
        .gooey-effect { position:absolute; opacity:1; pointer-events:none; display:grid; place-items:center; z-index:1; }
        .gooey-effect.text { color:white; transition:color 0.3s ease; font-family:var(--font-geist-mono),monospace; font-size:0.875rem; }
        .gooey-effect.text.active { color:#080808; }
        .gooey-effect.filter { filter:blur(7px) contrast(100) blur(0); mix-blend-mode:lighten; }
        .gooey-effect.filter::before { content:""; position:absolute; inset:-75px; z-index:-2; background:black; }
        .gooey-effect.filter::after { content:""; position:absolute; inset:0; background:white; transform:scale(0); opacity:0; z-index:-1; border-radius:9999px; }
        ...
        .gooey-li.active { color:#080808; text-shadow:none; }
        .gooey-li.active::after { opacity:1; transform:scale(1); }
        .gooey-li::after { content:""; position:absolute; inset:0; border-radius:8px; background:white; opacity:0; transform:scale(0); transition:all 0.3s ease; z-index:-1; }
      `}</style>
      <div className="relative" ref={containerRef}>
        <nav className="flex relative" style={{ transform: 'translate3d(0,0,0.01px)' }}>
          <ul ref={navRef} className="flex gap-2 list-none p-0 px-2 m-0 relative z-[3]" style={{ color: 'white', textShadow: '0 1px 1px hsl(205deg 30% 10% / 0.2)' }}>
            {items.map((item, index) => (
              <li key={index} className={`gooey-li rounded-full relative cursor-pointer transition-all duration-300 text-white font-mono text-sm ${activeIndex === index ? 'active' : ''}`}>
                <a href={item.href} onClick={e => handleClick(e, index)} className="outline-none py-[0.6em] px-[1em] inline-block">
                  {item.label}
                </a>
              </li>
            ))}
        ...
        <span className="gooey-effect filter" ref={filterRef} style={activeIndex < 0 ? { opacity: 0 } : {}} />
        <span className="gooey-effect text" ref={textRef} style={activeIndex < 0 ? { opacity: 0 } : {}} />
      </div>
    </>
  );
};
